...
### Example 3: Strongly Typed Event Dispatcher (Browser)
...
const bus = new EventDispatcher<AppEvents>();

// Subscribe
const unsubscribe = bus.on('progress', ({ current, total }) => {
  const percent = Math.round((current / total) * 100);
  console.log(`Progress: ${percent}%`);
});

// Emit safely (type-checked payloads)
bus.emit('progress', { current: 5, total: 20 });
bus.emit('userLogin', { userId: 'u123', name: 'Alice' });
